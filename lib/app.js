...
App.prototype.addFramework = function(framework) {
  ...
  ['combineScripts', 'combineStylesheets', 'minifyScripts', 'validateScripts', 'minifyStylesheets'].forEach(function(key) {
    if (this[key] === true) {
      framework[key] = true;
    }
  }, this);
...
App.prototype.manifest = function() {
  ...
  var that = this;
  // l.fs.readdir(that.savePath, function (err, files) {
  ...
  var scanner = new Scanner(this, function (files, rootDir) {
    ...
    console.info('ManifestPath', rootDir, manifestPath);
    l.fs.writeFile(manifestPath, cacheFiles, function(err) {
      if(err) {
        l.sys.puts(err);
      } else {
        l.sys.puts("The file was saved!");
      }
    }); 
